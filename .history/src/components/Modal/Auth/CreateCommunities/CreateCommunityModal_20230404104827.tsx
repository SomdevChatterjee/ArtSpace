...
const CreateCommunityModal: React.FC<CreateCommunitiesProps> = ({
	...
	const handleCreateCommunity = async () => {
		...
		try {
			//Create the communtiy document in firestore
			//check the name is valid or not 
			//if valid name then create community
	
			const communityDocRef = doc(firestore, "communities", communityName);
			
			await runTransaction(firestore, async (transactions) => {
				
				const communityDoc = await transactions.get(communityDocRef);
				
				//unique name validity
				if(communityDoc.exists()){
					throw new Error(`Oops!! A/${communityName} is already taken! Please try another!`);
				}
				
				//create community
				transactions.set(communityDocRef, {
					creator_Id:user?.uid,
				...
				//create the communtiy snippet on user

				transactions.set(doc(firestore, `users/${user?.uid}/communitySnippets`, communityName), {
					communityId:communityName,
					isModerator:true,
				})
			})
			handleClose()
			router.push(`/A/${communityName}/Index`);
