...
const usePosts = () => {
  ...
  const onVote = async ( event:React.MouseEvent<SVGElement>, post: Post, vote: number, communityId: string) => {
  ...
  const onSelectPost = (post:Post) => {
    ...
    router.push(`/A/${post.communityId}/comments/${post.id}`)
  };

  const onDeletePost = async (event:React.MouseEvent<HTMLElement> post: Post): Promise<boolean> => {
    try {
