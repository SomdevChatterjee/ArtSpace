...
const usePosts = () =>{
        ...
        try {
            ...
            //Existing vote[they  have posted before
                ...
                if(existingVote.voteValue === vote){
                    ...
                    updatedPost.votesStatus = votesStatus - vote;
                    updatePostVotes = updatePostVotes.filter(vote =>vote.id !== existingVote.id)
    ...
    const onDeletePost = async (post:Post): Promise<boolean> => {
      ...
      const getCommunityPOstVotes =async (communityId:string) => {
        const postVoteQuery = query(collection(firestore, 'users', `${user?.uid}/postVotes`), where('communityId', '==', communityId))
      }
    return{
        postStateValue,
        setPostStateValue,
